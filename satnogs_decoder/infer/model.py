...
Field spans are derived from the boundary head. The field heads classify those
spans as signed/unsigned and enum/non-enum.
"""
...
def field_features(pos_feats: np.ndarray, spans: list[tuple[int, int]]) -> np.ndarray:
    ...
    n_features = pos_feats.shape[1] * 3 + 1
    for s, e in spans:
